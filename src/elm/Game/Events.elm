...
eventHandler model event =
    let
        ...
        (account_, cmdAccount) = accountEventHandler model.account event
        (software_, cmdSoftware) = softwareEventHandler model.software event
        (server_, cmdServer) = serverEventHandler model.server event
        (network_, cmdNetwork) = networkEventHandler model.network event

        cmdList = [cmdMeta] ++ [cmdAccount] ++ [cmdSoftware] ++ [cmdServer]
                  ++ [cmdNetwork]
        cmdList_ = List.filter (\x -> List.member x cmdList) cmdList
