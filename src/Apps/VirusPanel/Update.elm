...
onSetModal : Maybe ModalAction -> Model -> UpdateResponse msg
onSetModal modal model =
    let
        model_ =
            { model | modal = modal }
...
doCollectWithBankRequest config bounceId bankAccountId virusList =
    let
        ...
        virusReducer nip acu =
            let
                activeVirus =
                    Dict.get nip (Database.getHackedServers config.database)
...
onHandleCollected config response model =
    let
        model_ =
            case response of
                Err error ->
                    { model | modal = Just <| ForError (CollectError error) }

                Ok () ->
                    { model | modal = Just <| ForCollectSuccessful }
    in
        ( model_, React.none )
