                ...
                Just ForCollect ->
                    case getCollectType config.database model.toCollectSelected of
                        ...
                        Just BitcoinVirus ->
                            { model
                                | modal = modal
                                , collectSelected = Just (BTC Nothing Nothing)
                            }

                        Just MoneyVirus ->
                            { model
                                | modal = modal
                                , collectSelected = Just (Money Nothing Nothing)
                            }
    ...
    let
        model_ =
            case response of
                Err error ->
                    { model
                        | modal = Just <| ForError (CollectError error)
                        , collectSelected = Nothing
                    }

                Ok () ->
                    { model
                        | modal = Just <| ForCollectSuccessful
                        , collectSelected = Nothing
                    }
    in
        ( model_, React.none )
