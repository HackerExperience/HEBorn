...
import Decoders.Bounces
    exposing
        ( createResponseMapper
        , updateResponseMapper
        , removeResponseMapper
        )
...
doCreateRequest ({ toMsg, accountId } as config) hackedServers bounce =
    config
        ...
        |> Cmd.map (createResponseMapper >> CreateRequest >> toMsg)
        |> React.cmd
...
doUpdateRequest ({ toMsg, accountId } as config) hackedServers id bounce =
    config
        ...
        |> Cmd.map (updateResponseMapper >> UpdateRequest >> toMsg)
        |> React.cmd
...
doRemoveRequest ({ toMsg, accountId } as config) id =
    config
        ...
        |> Cmd.map (removeResponseMapper >> RemoveRequest >> toMsg)
        |> React.cmd
