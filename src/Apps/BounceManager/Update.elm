...
onCreateRequest : Config msg -> Result Error () -> Model -> UpdateResponse msg
onCreateRequest config response model =
    let
        model_ =
            case response of
                ...
                Err error ->
                    case error of
                        BadRequest ->
                            { model | modal = Just (ForError "Bad Request") }

                        Unknown ->
                            { model | modal = Just (ForError "Unknown Error") }
    in
        ( model_, React.none )


onUpdateRequest : Config msg -> Result Error () -> Model -> UpdateResponse msg
onUpdateRequest config response model =
    let
        model_ =
            case response of
...
onRemoveRequest : Config msg -> Result Error () -> Model -> UpdateResponse msg
onRemoveRequest config response model =
    let
        model_ =
            case response of
...
doCreateRequest ({ toMsg, accountId } as config) hackedServers bounce =
    ...
    in
        config
            ...
            |> Cmd.map (Result.mapError errorMapper >> CreateRequest >> toMsg)
            |> React.cmd
...
doUpdateRequest ({ toMsg, accountId } as config) hackedServers id bounce =
    ...
    in
        config
            ...
            |> Cmd.map (Result.mapError errorMapper >> UpdateRequest >> toMsg)
            |> React.cmd
...
doRemoveRequest ({ toMsg, accountId } as config) id =
    ...
    in
        config
            ...
            |> Cmd.map (Result.mapError errorMapper >> RemoveRequest >> toMsg)
            |> React.cmd
