...
viewTabBuild ({ database, bounces, toMsg } as config) ( id, bounce ) model =
    ...
    in
        div [ class [ Super, Builder ] ]
            ...
            , div [ class [ Building ] ]
                [ modalHandler config model
                , renderLeftBox config path model
                , renderRightBox config path model
                ]
            , div [ class [ Buttons ] ]
                [ button
                    [ ForReset ( id, bounce )
...
modalHandler { toMsg, batchMsg } model =
    let
        name =
...
renderLeftBox : Config msg -> List Network.NIP -> Model -> Html msg
renderLeftBox config path model =
    div [ class [ Servers ] ]
...
renderRightBox config path model =
    div
        ...
        (renderEntries config path model)
...
renderAvailableServers ({ database } as config) path model =
    (Database.getHackedServers database)
        |> Dict.filter (\k _ -> not <| List.member k path)
        |> Dict.toList
        ...
        Just ( _, bounce ) ->
            if renaming then
                case bounceNameBuffer of
                    Just bufferedName ->
                        renderEditing config bufferedName
