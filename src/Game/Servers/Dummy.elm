...
dummy =
    ...
    in
        initialModel
            |> insert "gateway0" (dummyServer "Main" "192.168.0.16" meta0)
            |> insert "gateway1" (dummyServer "Secondary" "192.168.0.18" meta0)
            |> insert "remote0" (dummyServer "Pwned" "153.249.31.179" meta1)
            |> insert "remote1" (dummyServer "Rekt" "143.239.31.169" meta1)
...
dummyServer : String -> String -> ServerMeta -> Server
dummyServer name ip meta =
    { name = name
    , ip = ip
