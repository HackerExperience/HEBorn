...
onOpenApp config cid params model =
    ...
    in
        case maybeAppId of
            Just appId ->
                -- it's probably worth do restore the window unless we can't
                -- guarantee that the launch event will always reuse an
                -- existing window
                updateAppParams config appId params model

            Nothing ->
                launch config desktopApp maybeContext (Just params) model
...
onClickIcon config desktopApp model =
    ...
    in
        if shouldLaunch then
            launch config desktopApp (Just context) Nothing model_
        else
            React.update model_
